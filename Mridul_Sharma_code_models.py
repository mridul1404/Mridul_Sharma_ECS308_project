...
class classification():
    def __init__(self, path='/Users/mridu/PycharmProjects/Marketing Campaign/', clf_opt='lr', no_of_selected_features=None):
        self.path = path
    ...
    def classification_pipeline(self):
        ...
        if self.clf_opt == 'ab':
            ...
            clf_parameters = {
                ...
                'clf__max_depth': (17, 40, 50, 60),
                'clf__ccp_alpha': (0.004, 0.01, 0.05, 0.1),
            }
            # Logistic Regression
    ...
    def get_data(self):
        ...
        reader = pd.read_csv('marketing_trn_data.csv')
        feader = pd.read_csv('marketing_trn_class_labels.csv',names=["X","Y"])
        labels = feader['Y']
        # Select all rows except the ones belong to particular class'
        reader[' Income '] = reader[' Income '].str.replace(r'\D', '', regex=True)
        reader[' Income '] = reader[' Income '].astype(float)
        #OneHotEncoding
        reader = pd.get_dummies(reader, columns=['Education'],prefix=['Education'])
        reader = pd.get_dummies(reader, columns=['Marital_Status'], prefix=['Marital_Status'])
        #imputation
        mean_value = reader[' Income '].mean()
        reader[' Income '].fillna(value=mean_value, inplace=True)
        #reader[' Income '] = reader[' Income '].fillna(reader.groupby('Education')[' Income '].transform('mean'))
        leader = reader.drop(["Dt_Customer", "Year_Birth"], axis = 1)
    ...
    def classification(self):
        ...
        #    data, tst_cat = self.get_data('Marketing_tst_data.csv')
